...
@section('title', 'Facturación con Detalles')
...
@section('content')
    <form action="{{ route('reportfacturaciondetalle') }}" method="GET" id="reporteForm">
        <div class="card">
            <div class="card-header" style="background-color: #3498db; color: white; border-top-left-radius: 8px; border-top-right-radius: 8px; padding: 10px;">
                <h3 class="card-title">Reporte de Facturación</h3>
            </div>
            <div class="card-body">
                <div class="form-row">
                    ...
                    <div class="form-group col-md-3">
                        <label for="search_id_factura">Buscar por ID de Factura:</label>
                        <input type="text" class="form-control" id="search_id_factura">
                    </div>
                </div>
                <div class="table-responsive">
                    <table id="facturaciodetalle" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                <th>ID de Factura</th>
                                <th>Productos</th>
                                ...
                                <th>Estado</th>
                                <th>Cantidad</th>
                                <th>ID de Detalle de Factura</th>
                                <th>Precio</th>
                                <th>Importe</th>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#facturaciodetalle').DataTable({
                "paging": false,
                "info": false,
                "searching": true,
                "footerCallback": function(row, data, start, end, display) {
                    ...
                    $('#total_factura').text('C$ ' + total.toFixed(2));
                }
            });
            ...
            // Buscador por ID de Factura
            $('#search_id_factura').on('keyup', function() {
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex) {
                    ...
                    var fecha_factura = data[2]; // Índice 2 corresponde a la columna de fecha_factura

                    if (fecha_inicio === '' || fecha_fin === '') {
                        return true;
                    }

                    var fechaFactura = new Date(fecha_factura);
            ...
            $('#total_factura').text('C$ ' + initialTotal.toFixed(2));
        });
    </script>
@stop
