...
@section('title', 'Usuarios con más Ventas')
...
@section('content')
    <form action="{{ route('ReportePdfusuarioconmasventas') }}" method="GET" id="reporteForm">
        <div class="card">
            <div class="card-header" style="background-color: #3498db; color: white; border-top-left-radius: 8px; border-top-right-radius: 8px; padding: 10px;">
                <h3 class="card-title">Reporte de Usuarios con más Ventas</h3>
            </div>
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group col-md-3">
                        <label for="search_id_usuario">Buscar por Id Usuario:</label>
                        <input type="text" class="form-control" id="search_id_usuario">
                    </div>
                </div>
                <div class="table-responsive">
                    <table id="usuarioconmasventas" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                <th>Id usuario</th>
                                <th>Nombres</th>
  ...
  opacity: 1; /* Mostrar el fondo blanco al pasar el ratón */
    ...
    <script>
        $(document).ready(function() {
            var table = $('#usuarioconmasventas').DataTable({
                "paging": false,
                "info": false,
                ...
                "footerCallback": function(row, data, start, end, display) {
                    ...
                    $('#total_monto').text('C$ ' + total.toFixed(2)); // Mostrar el total en el footer
                }
            });

            // Buscador por Id Usuario
            $('#search_id_usuario').on('keyup', function() {
            ...
            $('#total_monto').text('C$ ' + initialTotal.toFixed(2));
        });
    </script>
@stop
